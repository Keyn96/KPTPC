    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Register(RegisterModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    WebSecurity.CreateUserAndAccount(model.login, model.Password, new {fio=model.fio,workExp=model.workExp, salary=model.salary});
                    WebSecurity.Login(model.login, model.Password);
                    Roles.AddUserToRole(model.login, "Storekeeper");
                    return RedirectToAction("Index", "Home");
                }
        ...
        public ActionResult Manage(ManageMessageId? message)
        {
            ViewBag.StatusMessage =
                ...
                : message == ManageMessageId.SetPasswordSuccess ? "Specify a password."
                : message == ManageMessageId.RemoveLoginSuccess ? "An external account is deleted."
                : "";
        ...
        public ActionResult Manage(LocalPasswordModel model)
        {
            ...
            if (hasLocalAccount)
            {
                if (ModelState.IsValid)
                {
                    ...
                    {
                        ModelState.AddModelError("", "Incorrect current password or invalid new password.");
                    }
                }
            }
            else
        ...
        public ActionResult ExternalLoginConfirmation(RegisterExternalLoginModel model, string returnUrl)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    Worker user = db.Workers.FirstOrDefault(u => u.login.ToLower() == model.UserName.ToLower());
                    // Проверка наличия пользователя в базе данных
                    if (user == null)
                    {
                        // Добавление имени в таблицу профиля
                        db.Workers.Add(new Worker { login = model.UserName });
                        db.SaveChanges();
                    ...
                    {
                        ModelState.AddModelError("UserName", "The login already exists. Enter a different login.");
                    }
                }
            }
